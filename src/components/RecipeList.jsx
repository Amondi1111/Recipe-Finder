...
function RecipeList({ recipes = [], isLoading, isSearched, addFavorite, favorites }) {
    if (isLoading) {
        return (
    ...
    if (isSearched && (!recipes || recipes.length === 0)) {
        return (
            ...
            <img
                    ...
                    <button
                       ...
                       className={`mt-2 w-full px-4 py-2 rounded text-white ${
                        isFavorited ? 'bg-gray-400 cursor-not-alllowed' : "bg-[#FF6347] hover:bg-[#9B3131]"
